...
export type PlayerMeta = {
  ...
  primary: string;
  secondary: string;
  /** ESPN player id — used to fetch full-body cutouts (shoes-to-head)
   *  from ESPN's CDN. NBA's own headshot endpoint is head-and-shoulders
   *  only, so without this we can never render a full body. */
  espnId?: number;
};

/** ESPN serves true full-body cutouts at this URL pattern. */
export function espnFullBodyUrl(espnId: number): string {
