...
/**
 * Detail card for the current clip. MAKE/MISS pill, series tag, player +
 * action, the four model inputs, and the headline xFG → result line.
 *
 * Stripped of decorative dots, gradients, and glow shadows so the numbers and
 * type are the focus.
 */
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="rounded-lg border border-white/10 bg-white/[0.025]"
      >
        <div className="px-6 md:px-8 pt-6 pb-7">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <motion.span
              ...
              className="text-[10px] uppercase tracking-[0.22em] text-white/50 font-mono"
            >
              {clip.series}
            </motion.span>
          ...
          <div className="mt-6 border-t border-white/8 pt-5">
            <motion.div
              ...
              className="text-[10px] uppercase tracking-[0.24em] text-white/40 mb-4 font-mono"
            >
              What the model saw
            </motion.div>
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-3">
              ...
              <InputRow label="Situation" value={clip.inputs.situation} delay={0.40} />
            </dl>
          </div>
...
function ResultPill({ made }: { made: boolean }) {
  const tone = made
    ...
    : {
        ...
        bg: "color-mix(in srgb, var(--nike-accent) 10%, transparent)",
        fg: "var(--nike-accent)",
      };
  return (
    <motion.span
      ...
      className="inline-flex items-center px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.2em]"
      style={{
...
function ShuffleButton({ onClick }: { onClick: () => void }) {
  return (
    <motion.button
      ...
      whileHover={{ y: -1 }}
      whileTap={{ scale: 0.96 }}
      className="ml-auto inline-flex items-center gap-2 rounded-full border border-white/15 px-3.5 py-1.5 text-[10px] uppercase tracking-[0.2em] text-white/70 hover:text-white hover:border-white/40 transition-colors"
