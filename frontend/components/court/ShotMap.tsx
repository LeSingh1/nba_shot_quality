...
export function ShotMap({
  ...
  return (
    <div
      ...
      style={{
        aspectRatio: tilted ? "5 / 3" : "500 / 470",
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Shots are drawn FIRST so the court lines paint on top of them.
              With ~10k league-wide shots, the paint / FT-circle / 3PT arc
              were getting completely buried by the dot cloud. Lines on top
              re-establishes the geometry without making the dots smaller.

              For high counts we drop motion.circle (each one runs an
              entrance + hover transition) and render plain SVG circles —
              motion machinery at this scale tanks the framerate. */}
          {shots.length > 600
            ? shots.map((s, i) => (
                <circle
                  key={i}
            ...
            : shots.map((s, i) => {
                ...
                const stagger = Math.min(0.6, i * 0.003);
                return (
                  <motion.circle
                    key={i}
                    cx={s.x}
          ...
          {/* Court lines + rim — drawn LAST so they sit on top of the shot
              cloud. Slightly higher opacity than before so they punch
              through even when the cloud underneath is dense. */}
          <HalfCourt stroke={tilted ? "rgba(255,255,255,0.65)" : "rgba(255,255,255,0.4)"} strokeWidth={2} />
          <circle
            cx={COURT.HOOP_X}
            cy={COURT.HOOP_Y}
            r={COURT.RIM_R + 0.5}
...
function ArcStreak({ arc, loopDuration }: { arc: Arc; loopDuration: number }) {
  ...
  return (
    <g>
      {/* Soft halo behind the main stroke — exaggerates the arc's presence
          without requiring a heavier blur filter. */}
      <motion.path
        d={arc.d}
        fill="none"
        stroke={ARC_GREEN}
        ...
        animate={{ pathLength: [0, 1, 1, 1], opacity: [0, 0.25, 0.25, 0] }}
        transition={{
