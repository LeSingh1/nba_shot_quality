...
function SingleArc({
  ...
  // drei's <Line> wraps Line2/LineMaterial — supports real pixel widths
  // (native <line> is hard-capped at 1px). Type is `any` because the actual
  // class lives in three-stdlib and isn't worth importing just for typing.
  ...
  // Initial vertex colors (all dark — they're repainted each frame via
  // geometry.setColors). drei's Line wants [r,g,b] tuples here.
  const initialColors = useMemo(
    () => arc.points.map(() => [0, 0, 0] as [number, number, number]),
    [arc.points],
  );
  // Scratch buffer reused each frame for the flat [r,g,b,r,g,b,…] colors
  // we hand to Line2's geometry.setColors().
  const colorBuf = useMemo(() => new Float32Array(arc.points.length * 3), [arc.points]);

  // One-time material tweaks the drei <Line> prop API doesn't expose directly.
  useEffect(() => {
  ...
  useFrame(() => {
    ...
    // and fades behind it (comet tail). Line2's geometry exposes
    // setColors([r,g,b,r,g,b,…]) which we feed via a reused scratch buffer.
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (behind >= 0 && behind <= COMET_LENGTH) {
        const k = 1 - behind / COMET_LENGTH;
        brightness = k * k;
      } else if (behind > COMET_LENGTH && phase !== "flying") {
        brightness = 0.22 * (1 - fadeK);
      } else if (behind > COMET_LENGTH && phase === "flying") {
        brightness = 0.26 * Math.max(0, 1 - (behind - COMET_LENGTH) / 18);
      }
      ...
      colorBuf[i * 3 + 0] = Math.min(1, r);
      colorBuf[i * 3 + 1] = Math.min(1, g);
      colorBuf[i * 3 + 2] = Math.min(1, b);
    }
    const lineObj = lineRef.current;
    if (lineObj?.geometry?.setColors) {
      // Line2's setColors accepts a plain array or Float32Array.
      lineObj.geometry.setColors(colorBuf as unknown as number[]);
    }
      ...
      >
        ...
        >
          ...
          <Suspense fallback={null}>
            ...
            {/* Arcs only make sense when we're highlighting makes (green).
                In residual mode the colors encode over/under-performance,
                not outcome — animating "made shots into the hoop" would
                conflict with that reading. */}
            {mode === "result" && <ShotArcs shots={shots} reduce={reduce} />}
            <Hoop />
          </Suspense>
          <OrbitControls
            target={[0, 1, 7]}
